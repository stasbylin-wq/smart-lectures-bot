...
def handle_audio(message):
    try:
        status_msg = bot.send_message(message.chat.id, "⏳ Файл лекции успешно получен! Скачиваю аудио и отправляю ИИ Whisper на расшифровку...")
        ...
        for i, chunk in enumerate(text_chunks):
            completion = groq_client.chat.completions.create(
                ...
                messages=[
                    # 🎯 ЖЕСТКАЯ НАСТРОЙКА ПРОТИВ ОТСЕБЯТИНЫ И ИИ-МУСОРА:
                    {"role": "system", "content": "Ты — строгий технический ассистент-стенографист. Перед тобой фрагмент расшифровки учебной лекции. Твоя задача — сделать КРАТКИЙ, ЕМКИЙ и СЖАТЫЙ конспект этой части на русском языке. СТРОГО ЗАПРЕЩЕНО: додумывать факты, добавлять отсебятину, писать фразы вроде 'Конспект фрагмента', 'Как я подошел к заданию', 'Техническая заметка' или комментировать свою работу. Пиши ТОЛЬКО чистые факты, термины и тезисы, которые прозвучали в тексте лекции. Главные термины выдели жирным шрифтом (используй теги <b> и </b> вместо звездочек), важные тезисы оформи буллитами."},
                    {"role": "user", "content": f"Вот текст фрагмента:\n\n{chunk}"}
                ]
            )
            final_notes.append(completion.choices[0].message.content)
            # Твоя проверенная задержка 60 секунд для стопроцентного обхода лимитов
            if i < len(text_chunks) - 1:
                time.sleep(60)
        
        result_text = f"📚 <b>ЦЕЛЬНЫЙ КОНСПЕКТ ЛЕКЦИИ</b> 📚\n\n" + "\n\n".join(final_notes)
        ...
        # Правильное разделение сообщений для Telegram с паузой 2 секунды (Защита от Flood Control)
        if len(result_text) > 4000:
            for x in range(0, len(result_text), 4000):
                bot.send_message(message.chat.id, result_text[x:x+4000], parse_mode="HTML")
                time.sleep(2)
        else:
